...
const statusConfig: Record<CampaignStatus, { label: string; variant: "secondary" | "default" | "outline" | "destructive"; className?: string }> = {
  ...
  generating: { label: "Generating", variant: "outline", className: "animate-pulse" },
...
const CampaignDetails = () => {
  ...
  // Separate source assets (uploaded by user) from generated ones
  const sourceAssets = assets.filter((a) => a.status === "pending_review" && a.content_url);
  const generatedAssets = assets.filter((a) => a.status !== "pending_review" || !a.content_url);

  const handleGenerate = () => {
    // Placeholder — will be wired to backend in a future phase
  };
