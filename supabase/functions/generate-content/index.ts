...
async function generateImage(prompt: string, width: number, height: number) {
  ...
  try {
    const taskId = await kieCreateTask(KIE_AI_API_KEY, "seedream/4.5-text-to-image", {
      prompt, aspect_ratio: aspectRatio, quality: "basic",
    });
...
async function generateCaption(platform: string, format: string, brandContext: any, intelligenceBrief: any): Promise<string> {
...
// ── Prompt Builders ──────────────────────────────────────────
function buildImagePrompt(platform: string, format: string, brandContext: any, intelligenceBrief: any): string {
  ...
  prompt += `Visual style: ${baseStyle}. Creative angle: ${angle}. `;
...
function buildVideoPrompt(platform: string, format: string, brandContext: any, intelligenceBrief: any): string {
  ...
  prompt += `Brand: "${brandContext.businessName || "luxury studio"}". Style: ${baseStyle}. Concept: ${angle}. `;
...
async function processAssetsInBackground(
  ...
  for (let i = 0; i < assets.length; i++) {
    ...
    try {
      ...
      // Generate caption
      const caption = await generateCaption(platform, format, brandContext || {}, intelligenceBrief || {});
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "edit") {
      ...
      EdgeRuntime.waitUntil((async () => {
        try {
          ...
          await supabase
            .from("generated_assets")
            .update({ content_url: result.url, provider: result.provider, generation_cost: result.cost, generation_time_ms: result.timeMs, rationale: `Edited: ${editPrompt}`, status: "pending_review" })
            .eq("id", assetId)
            .eq("profile_id", userId);
          console.log(`[SeedEdit 3.0] Edit complete for asset ${assetId}`);
        } catch (e) {
          console.error("[SeedEdit 3.0] Edit failed:", e);
          await supabase
            .from("generated_assets")
            .update({ rationale: `Edit failed: ${e instanceof Error ? e.message : "Unknown"}`, status: "pending_review" })
            .eq("id", assetId);
        }
      })());

      return new Response(JSON.stringify({ success: true, message: "Edit started, processing in background" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    // Insert placeholder rows for all assets immediately
    ...
    for (const asset of assets) {
      ...
      if (insertError) {
        ...
        placeholderIds.push("error");
      } else {
        placeholderIds.push(placeholder.id);
      }
    }

    // Fire background processing via EdgeRuntime.waitUntil
    EdgeRuntime.waitUntil(
      processAssetsInBackground(userId, campaignId, assets, researchId || null, intelligenceBrief || {}, brandContext || {}, placeholderIds)
